...
calcCoutNoria params dataTrajet  = TrajetCout [trajet dataTrajet] coutLocation coutCarburant coutCO2 emissionsCO2 coutAchat coutMelange coutStockage coutAnalyse (location dataTrajet) (carburant dataTrajet)
   where coutLocation = prixDemiJournee  *  fromIntegral (ceiling nbDemiJoursNecessaires)
         prixDemiJournee = case typesite $ depart $ trajet dataTrajet of
           (Gi gisement) -> (camionPrixLocation $ gCamion gisement) / 2
           (Pl (Plateforme _ _ _ camion _)) -> (camionPrixLocation camion) / 2
           (Pl (PlateformeSMat _ _ _ camion)) -> (camionPrixLocation camion) / 2
           otherwise -> 0
...
nbAllerRetours quantite camion materiau =
  case quantite of
    (Volume volume) -> case allerRetourTonnage of
                          ...
                          where allerRetourTonnage = multScal (1/(camionTonnageMax camion)) <$> (enTonne quantite materiau)
                                allerRetourVolume  = (ceiling (volume / (camionVolumeMax camion))) 
    (Tonne tonne) -> case allerRetourVolume of
                           (Just (Volume x)) -> max (ceiling x) allerRetourTonnage
                           otherwise         -> allerRetourTonnage
                          where allerRetourVolume = multScal (1/(camionVolumeMax camion)) <$> (enVolume quantite materiau)
                                allerRetourTonnage = (ceiling (tonne / (camionTonnageMax camion)))
...
calculerCarburant quantite camion materiau km = ((fromIntegral (nbAllerRetours quantite camion materiau) * km * 2) / 100 ) * (camionConso camion)
